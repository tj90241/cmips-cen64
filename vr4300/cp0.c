...
static const uint64_t vr4300_cp0_reg_masks[32] = {
  ...
  0x00000000FFFFFFFFULL, // 15; VR4300_CP0_REGISTER_PRID
  0x00000000FFFFFFFFULL, // 16: VR4300_CP0_REGISTER_CONFIG
...
int VR4300_MFC0(struct vr4300 *vr4300, 
  ...
  if (iw & 0x7) {
    if (src == (VR4300_CP0_REGISTER_CONFIG - 32)) {
      exdc_latch->result = 0x1e190c8a;
    }
  }

  else if (src == (VR4300_CP0_REGISTER_WATCHLO - 32) ||
          (src == (VR4300_CP0_REGISTER_WATCHHI - 32))) {
    exdc_latch->result = 0;
  }

  else if (src == (VR4300_CP0_REGISTER_COUNT - 32)) {
    exdc_latch->result = (uint32_t)
      (vr4300->regs[VR4300_CP0_REGISTER_COUNT] >> 1);
  }
